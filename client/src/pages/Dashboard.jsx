...
import { Boxes, Package, Plus, Layers } from 'lucide-react';
...
export function Dashboard() {
  ...
  const [poSkus, setPoSkus] = useState([]);
  const poLookupTimer = useRef(null);

  async function load() {
    const data = await api('/api/sows');
    setSows(data.sows);
  }
    ...
    poLookupTimer.current = setTimeout(async () => {
      ...
      try {
        const [data, sowData] = await Promise.all([
          api(`/api/po-clients/lookup/${encodeURIComponent(po)}`).catch(() => null),
          api(`/api/sows/next-number?poNumber=${encodeURIComponent(po)}`),
        ]);
        ...
        setPoSkus([]);
        if (po.includes('-')) {
          toast.error(err.message || 'Could not generate SOW number');
        }
      }
    }, 300);
  }
  ...
  async function confirmSow() {
    ...
    setBusy(true);
    try {
      const data = await api('/api/sows', { method: 'POST', body: form });
