...
function packingTypeLabel(type) {
...
async function nextSowNumber(poNumber) {
...
sowsRouter.get('/', requireRole('admin', 'worker'), async (req, res) => {
  const { status } = req.query;
  const filter = {};
  if (status) filter.status = status;
  const sows = await Sow.find(filter).sort({ createdAt: -1 }).populate('createdBy', 'username role');
  res.json({ sows: await withTotals(sows) });
});

sowsRouter.get('/history', authRequired, requireRole('admin'), async (_req, res) => {
  const sows = await Sow.find({ status: 'completed' })
    .sort({ completedAt: -1 })
    .populate('createdBy', 'username')
    .populate('completedBy', 'username');
  const withStats = await withTotals(sows);
...
sowsRouter.get('/next-number', requireRole('admin', 'worker'), async (req, res) => {
  ...
  try {
    ...
    res.status(err.status || 500).json({ message: err.message || 'Failed to generate SOW number' });
  }
});

sowsRouter.get('/:id', requireRole('admin', 'worker'), async (req, res) => {
...
sowsRouter.post('/', requireRole('admin', 'worker'), async (req, res) => {
  ...
  const skus = Array.isArray(selectedSKUs) ? selectedSKUs.filter(Boolean) : [];
  if (type === 1 || type === 2) {
    if (skus.length !== 1) {
  ...
  try {
    ...
    return res.status(err.status || 400).json({ message: err.message || 'Invalid PO number' });
  }
...
sowsRouter.post('/:id/save', requireRole('admin', 'worker'), async (req, res) => {
  const sow = await Sow.findById(req.params.id);
  if (!sow) return res.status(404).json({ message: 'SOW not found' });
...
sowsRouter.post('/:id/complete', requireRole('admin', 'worker'), async (req, res) => {
  const sow = await Sow.findById(req.params.id);
  if (!sow) return res.status(404).json({ message: 'SOW not found' });
  if (sow.status === 'completed') {
    return res.status(400).json({ message: 'This SOW is already completed' });
