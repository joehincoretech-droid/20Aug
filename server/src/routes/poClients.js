...
poClientsRouter.get('/lookup/:poNumber', async (req, res) => {
...
poClientsRouter.get('/', requireRole('admin'), async (_req, res) => {
  const mappings = await PoClient.find().sort({ poNumber: 1 });
  res.json({ mappings });
});

poClientsRouter.post('/', requireRole('admin'), async (req, res) => {
